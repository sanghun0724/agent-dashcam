...
- **Worst-session resume hints** — daily + weekly markdown and Slack payloads now surface a "Sessions that need a look" section listing every score with `weighted_avg < worst_sessions_threshold` (default `0.5`), capped at `worst_sessions_max` (default `3`), sorted worst-first. Each entry includes the full `sessionId` plus a provider-aware resume command (`claude --resume <id>` / `codex --resume <id>` / a `~/.gemini/tmp` file-path hint for Gemini) so a Slack reader can one-click jump back into the problematic session instead of digging through `scores/`. Config-driven via `{daily,weekly}_report.{show_worst_sessions, worst_sessions_threshold, worst_sessions_max}`; `score_jsonl()` now stamps `meta.provider` so the hint matches the CLI that produced the session. 21 new tests in `fixtures/test_daily_report.py` covering threshold filter, max cap, worst-first sort, sessionId skip, `_lowest_axis_for_score` suppression handling, and all provider templates (claude/codex/gemini/unknown/None-fallback). Total: 162 tests.
- **Per-subagent token attribution** (`scripts/adapters/claude.py:_collect_agent_attribution`) — `load_session()` now returns an `agent_attribution` dict mapping each `subagent_type` (e.g. `"architect"`, `"Explore"`, `"oh-my-claudecode:critic"`) to `{total_tokens, tool_uses, duration_ms, call_count}`. Built by pairing every `Agent` / legacy `Task` `tool_use` (keyed by `id` → `input.subagent_type`) with its matching `tool_result`, then regex-scanning the result text for the `<usage>total_tokens: N tool_uses: M duration_ms: D</usage>` block that Claude Code subagents embed. Surfaces as `meta.agent_attribution` in the score JSON so downstream consumers can compare delegation cost across subagent roles without cross-referencing sidechain JSONLs. Call-count increments even when the `<usage>` block is absent, so subagent invocations that returned early are still visible. 8 new tests in `fixtures/test_adapter_claude.py` (`AgentAttributionTests`): Agent/Task bucketing, repeated-call aggregation, missing-subagent-type skip, content-as-string tolerance, end-to-end via `NamedTemporaryFile`. Total: 170 tests.
...
- **Per-subagent attribution is token-count only, not input/output/cache split** — `_collect_agent_attribution` parses `total_tokens` from the subagent's `<usage>` summary block, which does not break down into `input_tokens` / `output_tokens` / `cache_read_input_tokens` / `cache_creation_input_tokens`. Per-subagent **cost** therefore cannot be priced accurately (cache reads are ~10× cheaper than fresh inputs; without the split we can only estimate an upper bound). To fix this properly, the adapter would need to walk the sidechain JSONL for each `subagent_type` and pull the per-turn `message.usage` block. Flagged as a follow-up; the current aggregate token count is already enough to compare relative delegation load across subagent roles.
- **Invoice reconciliation remains manual** — even with per-subagent attribution, the Anthropic / OpenAI / Google billing export does not carry a `session_id` or `subagent_type` column, so mapping a scored session (or a per-subagent bucket) back to an invoice line item is still a by-hand diff. This is an upstream vendor limitation, not something agent-dashcam can close on its own.
